...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $date = $_POST['date'];
    $phone = $_POST['phone'];
    $selected_ritual = $_POST['selected_ritual'];

    // создаем переменную с содержанием письма
    $content = $name . ' оставил заявку на Spa-ритуал: ' . $selected_ritual . ' На время: ' . $date . '. Его телефон: ' . $phone;

    // Первый параметр - кому отправляем письмо, второй - тема письма, третий - содержание, четвертый - содержание
    $headers  = 'MIME-Version: 1.0' . "\r\n";
    $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
    $success = mail("admin@whitelotus.com", 'Запрос на Spa-ритуал', $content, $headers);
